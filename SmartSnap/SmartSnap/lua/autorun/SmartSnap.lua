...
local function OnShutDown()
	output = file.Read('smartsnap_offsets_custom.txt', "data")
	if output == nil then
...
local function DrawGridLines(vOrigin, vSX, vSY, gridLines, offsetX, offsetY, sign)
	...
	local vOffset, temp
	local vsNormal = (ToScreen(vX) - ToScreen(vY)):GetNormal()
...
local function DrawBoundaryLines(vOrigin, vOpposite)
	...
	if (vOrigin:Distance(vOpposite) > 5) then
		vPoint = vOrigin + (vOpposite - vOrigin):GetNormal() * 5
	else
...
local function OnPaintHUD()
	...
	if (!target.locked) then
		...
		local trace = util.TraceLine({
			start  = target.entity:LocalToWorld(target.entity:WorldToLocal(vectorGrid) - vectorOffset) - vectorZ:GetNormal() * 0.01,
